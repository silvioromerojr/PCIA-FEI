/*PROGRAMACAO CIENTIFICA - PEL216
 ...
 PROGRAMA PARA A FUNCAO 2 sqrt(1 - x^2) */
#include <cmath>
#include <stdlib.h>
#include <iostream>
#include <iomanip>
...
{
    return - (1 / (sqrt ( 1 - pow(x , 2)))) - (pow(x , 2) / pow(1 - pow(x , 2), 3/2));
...
{
    return - (pow(x , 2) / pow(1 - pow(x , 2), 3/2)) - (18 * (pow(x , 2)) / pow(1 - pow(x , 2), 5/2)) - (15 * (pow(x , 4)) / pow(1 - pow(x , 2), 7/2));
}
...
int main()
{
    ...
    cout << "Solucao analitica:   0.785398\n" << endl;
